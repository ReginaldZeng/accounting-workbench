...
def _xlsx(res, month_label="", punch_bytes=None):
    ...
    PARAMS = [
        ...
        ("多记弹性（小时）", p["tolerance"], "按人整期净多记（判过班日的 上报−在厂 合计）在此值以内视为正常波动；超过才判异常。与成本会计一致，不逐日判"),
    ...
    title(ws3, "④ 逐日核对 — 按口径重算每一天", 14,
          ...
          "判定按**人整期**算（与成本会计一致）：某人整期净多记超弹性 → 那几天「⚠整期超弹性,异常」；"
          "逐日冒尖但本人整期没超 → 「○整期在弹性内,已消化」中性不查。底色：红＝异常、黄＝少记。")
    ...
    for x in res["rows"]:
        ...
        _tail = (f'IF(AND(M{r}<-0.001,{_pnet}>{C_TOL}+0.001),"⚠ 多记 "&TEXT(-M{r},"0.0")&" 小时(撑不起上报，整期超弹性)",'
                 f'IF(M{r}<-0.001,"○ 多记 "&TEXT(-M{r},"0.0")&" 小时(撑不起上报，整期已消化)",'
        ...
        # 同名待指认：归一后撞上多个人、工具没定人 → 匹配不到唯一打卡行（I=0）。
        # 判定公式那层 AND(I=0,K>0) 会把它打成「⚠ 记了工时但无打卡」冤枉员工，
        # 所以像白夜混合一样改写**内核算好的静态判定**（◇ 中性，不上色），本行也无从随参数联动重算。
        if x.get("档") == "ambig":
            ws3.cell(r, 14, x.get("判定") or "◇ 同名待指认（成本会计按钉钉部门判断）")
        r += 1
    LAST3 = r - 1
    if LAST3 >= 4:
    ...
    head(ws1, r, ["判定档", "日次 / 人数", "工时(小时)", "金额(元)", "含义"], [30, 11, 13, 13, 72]); r += 1
    _bands = ([("✓ 打卡撑得住上报", st["一致日次"], "—", "—",
               ...
               ("⚠ 撑不起上报（整期超弹性）", st.get("超弹性人数") or 0, st["异常多记小时"], st["异常多记金额"],
                "唯一必须查的一档，按人整期判（与成本会计一致）：某人整期净多记超弹性、公司整期多付。数字＝超弹性人数 / 整期净多记小时·金额。须业务逐笔说明（多为漏打卡）")]
              if _BASIS != "punch" else
    ...
    for a, n, h, m, note in _bands + [
        ...
        ("◇ 同名待指认（成本会计判断）", st.get("同名待指认日次", 0), "—", "—",
         "报了工时、但钉钉有俩同名、工具不猜；每个候选的钉钉部门已列在打卡表与结算风险页，"
         "「临时普工-…人力」才是临时工。成本会计定人后这些天才谈工时——不是员工没打卡"),
