...
def _xlsx(res, month_label="", punch_bytes=None):
    ...
    C3 = ["姓名", "部门", "归属", "岗位", "班型", "日", "上班打卡", "下班打卡", "打卡\n次数",
          "跨度\n(小时)", "上报\n工时", "重算\n工时", "差异\n(重算-上报)", "判定", "已认定\n（人·时间·理由）", "键",
          "单价\n(含管理费)", "金额影响\n(差异×单价)", "无效卡\n(上下班之外)"]
    # ⚠ 新列追加在隐藏键列 P 之后（Q、R、S），**不插在中间**——③页的 SUMIFS/COUNTIFS 按列字母引用
    #    K/L/M/N/P，插一列就全错位了。无效卡本该挨着上下班，但受此约束只能摆最后。
    # 上/下班打卡列给 13：夜班次日下班显示成「次日 08:33」，窄了会撑成 ########
    head(ws3, 3, C3, [12, 9, 8, 7, 7, 5, 13, 13, 7, 9, 9, 9, 11, 30, 28, 4, 11, 13, 16])
    ...
    for x in res["rows"]:
        ...
        _tail = (f'IF(M{r}<-{C_TOL}-0.001,"⚠ 多记 "&TEXT(-M{r},"0.0")&" 小时(打卡撑不起上报,异常)",'
