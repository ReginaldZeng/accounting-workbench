...
bom_quote_entry = Table(
    ...
    Column("raw_version", Text),                      # 财务复核顶替商务输出时，商务输出留作采购原始参考 JSON：{srcLabel,full,五分项}（V2.547）
    Column("final_return", Text),                     # 终审退回留痕 JSON：{by,at,note,via(工作台/OA)}（V2.584）；重新初审清空
    Column("unfinal_req", Text),                      # 撤回终审申请 JSON：{state,by,at,reason,decidedBy,decidedAt,note,mode}（V2.585）
...
_BOM_JSON_COLS = ("summary", "materials", "checks", "src_fee", "bom_list", "craft", "review_steps", "goods_version", "void_req", "ack", "recalc", "manual_upstream", "raw_version", "final_return", "unfinal_req")
...
def _ensure_bom_columns():
    ...
    try:
        ...
        for col in ("bom_list", "review_steps", "origin", "src_label", "goods_version",
                    ...
                    "inactive_kind", "void_req", "ack", "stale_note", "obsolete_at", "obsolete_note", "variant_group", "recalc", "manual_upstream", "raw_version", "final_return", "unfinal_req"):
            if col not in cols:
                with _engine.begin() as c:
