  ...
  return <div className="ew-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="ew-drawer ec-chain-drawer" role="dialog" aria-modal="true" aria-labelledby="order-drawer-title" tabIndex="-1" ref={ref}>
    ...
    {result.error?<p role="alert">读取失败：{result.error}。请关闭后重试。</p>:!r?<p role="status">正在读取已保存订单证据…</p>:<>
      ...
      <div className="ew-scroll ec-chain-scroll"><table className="ec-chain-table"><thead><tr><th>业务环节</th><th>日期／时间</th><th>关联单据／账户</th><th>金额／数量</th><th>核对状态</th></tr></thead><tbody>
        ...
        {r.price_protections?.length>0&&<ChainRow name="天猫价保赔付" date={<DateRange label="申请时间" values={r.price_protections.map(p=>p.date)}/>} summary={`${r.price_protections.length} 条价保申请 · 不重复增加退款或费用`} status="已关联，扣款以流水为准" initialOpen={initialTab==='price'}>
          <p>价差金额与逆向退款金额分别展示；逆向退款可能含退还天猫积分类服务费，不将两列相加。自动通过不等于已核实账户扣款。</p>
          <div className="ew-scroll"><table><thead><tr><th>子订单号／来源</th><th>申请时间</th><th>价差金额</th><th>逆向退款金额（含退还天猫积分类服务费）</th><th>原始状态</th></tr></thead><tbody>{r.price_protections.map((p,i)=><tr key={p.application_key||i}><td>{p.suborder_no}<small>{p.source_filename||'价保来源'} · {p.sheet} · 第 {p.row_number} 行</small></td><td>{p.date}</td><td>¥ {money(p.price_difference)}</td><td>{p.reverse_refund==null?'未提供':`¥ ${money(p.reverse_refund)}`}{p.reverse_note&&<small>{p.reverse_note}</small>}</td><td>{p.status}</td></tr>)}</tbody></table></div>
        </ChainRow>}
