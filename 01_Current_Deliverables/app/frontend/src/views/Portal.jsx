...
import PortalAcceptance from './PortalAcceptance.jsx'   // V2.499 门户跨台验收台账
import VerifyPopup from './VerifyPopup.jsx'             // V2.499 登录验收弹窗（系统语气）
...
export default function Portal({ user, onEnter }) {
  ...
  const tabOf = { '工作台门户': 'home', '验收台账': 'accept', '使用指南': 'guide', '模型配置': 'model', '账号管理': 'admin', '门户管理': 'cms' }
  ...
  return (
    <div className="pt-root">
      ...
      {/* 登录验收弹窗（V2.499）：不分页签，我名下有待验收就弹（系统语气）；稍后只关不改状态，下次登录再弹 */}
      <VerifyPopup />
    </div>
  )
}
